document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Function to fetch crypto to USD exchange rate from CoinGecko
    async function fetchCryptoToUsdExchangeRate(crypto) {
        try {
            const response = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${crypto}&vs_currencies=usd`);
            const data = await response.json();

            if (data && data[crypto] && data[crypto].usd !== undefined) {
                return parseFloat(data[crypto].usd);
            } else {
                console.error('Invalid exchange rate data:', data);
                throw new Error(`Exchange rate data for ${crypto} is not available or has an unexpected format.`);
            }
        } catch (error) {
            console.error(error);
            throw new Error(`Error fetching exchange rate for ${crypto}: ${error.message}`);
        }
    }
    ...
    document.getElementById('swapButton').addEventListener('click', async () => {
        ...
        if (!web3) {
            alert("Please connect your wallet (e.g., MetaMask) before swapping.");
            return;
        }

        try {
            // Fetch exchange rates from CoinGecko
            const fromCryptoToUsd = await fetchCryptoToUsdExchangeRate(fromCrypto);
            const toCryptoToUsd = await fetchCryptoToUsdExchangeRate(toCrypto);

            // Calculate the equivalent amount in USD
            const fromCryptoAmountUsd = swapAmount * fromCryptoToUsd;
            const toCryptoAmount = fromCryptoAmountUsd / toCryptoToUsd;

            // For simplicity, redirect to MetaMask swap page (replace with the appropriate URL for other wallets)
            redirectToWalletSwapPage(fromCrypto, toCrypto);

            alert(`Successfully redirected to swap ${swapAmount} ${fromCrypto} to ${toCryptoAmount.toFixed(6)} ${toCrypto}`);
        } catch (error) {
            console.error(error);
    ...
    // Function to redirect to wallet's swap page
    function redirectToWalletSwapPage(fromCrypto, toCrypto) {
        // Adjust this function based on the specific requirements of the wallet extension
        // This example assumes a simplified case with a generic URL
        const swapUrl = `https://wallet-extension.com/swap?from=${fromCrypto}&to=${toCrypto}`;
